...
function askCancel(id) { const r = reservations.find((x) => x.id === id); if (!r) return; pendingCancel = id; $('cancel-copy').textContent = `Cancel “${r.topic}” in ${r.room} on ${r.date}? The record will be permanently deleted.`; $('cancel-dialog').showModal(); }

...
function validate(item, id = '') {
  ...
  const invalidOrder = item.start && item.end && item.end <= item.start;
  const tooLong = item.start && item.end && minutes(item.end) - minutes(item.start) > 120;
  timeError.textContent = invalidOrder ? 'End time must be after start time.' : tooLong ? 'End time must be within 2 hours of start time.' : '';
...
function askCancel(id) { const r = reservations.find((x) => x.id === id); if (!r) return; pendingCancel = id; $('cancel-copy').textContent = `Cancel “${r.topic}” in ${r.room} on ${r.date}? The record will remain visible.`; $('cancel-dialog').showModal(); }
...
$('cancel-dialog').addEventListener('close', () => { if ($('cancel-dialog').returnValue === 'confirm' && pendingCancel) { reservations = reservations.map((r) => r.id === pendingCancel ? { ...r, status: 'cancelled' } : r); save(); render(); } pendingCancel = null; });
render();

...
function validate(b, ignoreId = '') {
  ...
  if (minutes(b.end) - minutes(b.start) > 120) { endError.textContent = 'End time cannot be more than 2 hours after start.'; document.querySelector('#end').focus(); return false; }
...
function render() {
  list.replaceChildren(); const active = bookings.filter((b) => b.lifecycle === 'active').length; document.querySelector('#count').textContent = `${active} active · ${bookings.length} total`; empty.hidden = bookings.length > 0;
  [...bookings].sort((a,b) => `${a.date}${a.start}`.localeCompare(`${b.date}${b.start}`)).forEach((b) => { const li = document.createElement('li'); li.className = `booking ${b.lifecycle}`; const title = document.createElement('h3'); title.textContent = b.topic; const meta = document.createElement('p'); meta.className = 'meta'; meta.textContent = `${b.date} · ${b.start}–${b.end} · ${b.room} · ${b.person}`; const badge = document.createElement('span'); badge.className = 'badge'; badge.textContent = b.lifecycle === 'active' ? 'Active' : 'Cancelled'; li.append(title, meta, badge); if (b.lifecycle === 'active') { const actions = document.createElement('div'); actions.className = 'booking-actions'; const edit = document.createElement('button'); edit.type = 'button'; edit.textContent = 'Edit'; edit.addEventListener('click', () => editBooking(b.id)); const cancel = document.createElement('button'); cancel.type = 'button'; cancel.className = 'secondary'; cancel.textContent = 'Cancel'; cancel.addEventListener('click', () => askCancel(b.id)); actions.append(edit, cancel); li.append(actions); } list.append(li); });
}

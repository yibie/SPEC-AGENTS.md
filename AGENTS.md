...
**行动**：
1.  **读取**：`.phrase/modules/pr_faq.md`
2.  **执行**：扮演“严格的产品经理”角色。进行访谈以起草亚马逊风格的 PR/FAQ。
3.  **约束**：在 PR/FAQ 最终确定之前，禁止开始编写代码或拆解任务。

## 2. 🔨 编码 / 重构 / 审查
**触发条件**：用户请求代码实现、Bug 修复、重构或代码审查。
**行动**：
1.  **读取**：`.phrase/modules/linus_coding.md`
2.  **执行**：扮演“Linus Torvalds”角色。
3.  **约束**：在编码前和编码过程中，严格执行“5 层思考模型”和“好品味”判断。
...
## 2. Phase 工作流
1. **Phase Gate**（仅当用户明确开启新阶段）：在新 `phase-*` directory 创建最小集 `spec_*`, `plan_*`, `task_*`, 视需求补 `tech-refer_*`/`adr_*`，`issue_*` 可后置。
2. **In-Phase Loop**（默认）：  

...
读取 `AGENTS.md`、`.spec-agents/doctrine/docs/WORKFLOW.md`、`CONTEXT.md`、项目
`.spec-agents/state/KERNEL.md`（若存在）、`.spec-agents/state/STATUS.md`、相关 `SPEC.md`、一个目标 Slice 和必要的
...
拒绝的 ticket。读取 `AGENTS.md`、`.spec-agents/doctrine/docs/WORKFLOW.md`、
`CONTEXT.md`、项目 `.spec-agents/state/KERNEL.md`（若存在）、`.spec-agents/state/STATUS.md` 和必要的 Protocol、
Runbook、Lesson。确认：
...
动手前把这次要写的位置，对照项目 `.spec-agents/state/KERNEL.md` 的 Architecture boundaries
...
第二种情况刻意不阻断：存量 Kernel 本来就不要求回填这张图（见 `.spec-agents/doctrine/START.md`），
...
- 回到 `plan`，不直接修改 `.spec-agents/state/KERNEL.md`、`CONTEXT.md`、ADR、Protocol、Runbook、Lesson 或 SPEC。
...
- `.spec-agents/state/KERNEL.md`、`CONTEXT.md`、`.spec-agents/state/STATUS.md`、`.spec-agents/state/EVIDENCE.md` 和
  `docs/{adr,protocols,runbooks,lessons}/` 由 `learn` 写入；
- `.spec-agents/specs/<feature>/SPEC.md` 由 `capture` 写入，Slice 由 `arrange` 创建；
  `do` 在任何路径上都不创建 Slice；Slice 的 `done`/`evidence_ref` 和 SPEC 的
  `verified` 由 `learn` 写入；
- 被管项目中，安装进来的 doctrine（`AGENTS.md`、`.spec-agents/doctrine/START.md`、`.spec-agents/doctrine/UPGRADE.md`、
  `.spec-agents/doctrine/skills/`、`.spec-agents/doctrine/docs/`）任何动作都不写——本地修改会被下一次安装
  覆盖，要改就改上游。

...
Everything under `.spec-agents/doctrine/` is framework material. It is identical in every
...
`.spec-agents/doctrine/AGENTS.md`, `.spec-agents/doctrine/START.md`,
`.spec-agents/doctrine/UPGRADE.md`, and `.spec-agents/doctrine/skills/`. All five are installed,
...
- `check-kernel.sh` — the only executable here. Verifies the *form* of this
  ...
  second site. Run it as `.spec-agents/doctrine/docs/check-kernel.sh .`. It does not check
...
Your project's own knowledge does not live here. It lives in
`.spec-agents/state/KERNEL.md`, `CONTEXT.md`, `.spec-agents/state/STATUS.md`,
`.spec-agents/state/EVIDENCE.md`, and the knowledge

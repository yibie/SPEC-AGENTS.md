...
`.spec-agents/doctrine/docs/WORKFLOW.md`。被管项目自己的语义在 `.spec-agents/state/KERNEL.md`，项目自己的
...
Start 对项目 `.spec-agents/state/KERNEL.md` 的状态标记：`present`、`K1-bootstrap`、`stale`、
`contradicted`、`kernel-unavailable`。与 ProjectState 分开，避免把「入口可用」
和「语义已建立」混成一个状态。

`stale` 和 `contradicted` 由 `start` 的 re-scan 产出——那是唯一会重新扫描
现实的动作，它不修改 `.spec-agents/state/KERNEL.md`，也不自行路由（ADR 0005）。
...
由安装器发出、在每个被管项目里内容相同的框架材料：`.spec-agents/doctrine/{AGENTS.md,START.md,UPGRADE.md,skills/,docs/}`；不含任何单个仓库的实例事实（ADR 0001）。

### Project integration entry

被管项目根 `AGENTS.md` 中连接自动发现与完整 Doctrine 的最小入口；可以是安装器生成的精确薄适配器或项目自己的指令，根文件存在或宽松提到 SPEC-AGENTS 都不证明集成完成，项目自己的入口不属于 Doctrine 且安装器不得覆盖。

### Instance

某一个仓库自己的状态与知识：`.spec-agents/state/STATUS.md`、`.spec-agents/state/EVIDENCE.md`、`.spec-agents/state/KERNEL.md`、根
`CONTEXT.md`、`.spec-agents/specs/`、`docs/{adr,protocols,runbooks,lessons}/`、`.spec-agents/archive/`；永不安装到另一个项目（ADR 0001）。

### Project Kernel

被接入项目的稳定语义层：概念、身份、关系、动作契约、生命周期、不变量和少量
架构边界。WORKFLOW 描述框架自身的工作流语义，`.spec-agents/state/KERNEL.md` 描述被管项目的本体。
...
跨上下文工作的活的设计契约，持久保存在 `.spec-agents/specs/<feature>/SPEC.md`：已确认的
...
delta：工作验证后，将在项目 `.spec-agents/state/KERNEL.md`（本仓库为 `WORKFLOW.md`）中新增、修订、
...
一个新上下文可以完成、可以独立验证的工作单元，记录在 `.spec-agents/specs/<feature>/issues/`。Slice 不拥有本体定义，也不能绕过 Plan 修改长期规则；完成验证后可以带一个可选的 `evidence_ref` 指向 Evidence。

本次十项声明的来源映射恰为 Doctrine、Instance、Project integration entry、Start、Upgrade、Project Kernel、State、Evidence、SPEC、Slice ← upstream SPEC-AGENTS namespaced-project-layout SPEC r3；现有 upstream SPEC-AGENTS Evidence 为 `E-20260831-011`、`E-20260831-012`、`E-20260831-013`、`E-20260901-001`、`E-20260901-002`。
...
| --- | --- | --- |
| Semantic rule | 概念、身份、关系、生命周期和不变量 | 项目 `.spec-agents/state/KERNEL.md`；框架工作流为 `.spec-agents/doctrine/docs/WORKFLOW.md` |
...
`.spec-agents/state/EVIDENCE.md` 保存尚未提升的动态事实；`.spec-agents/state/STATUS.md` 保存当前活跃的工作。仓库
...
Project integration entry --resolves_to--> Doctrine
...
`.spec-agents/doctrine/skills/learn/SKILL.md`，不在这里复述。`superseded` 是另一条出口——被另一份
SPEC 取代——两者不是同义词。
...
`semantic` 发现引出；两者都不修改 `.spec-agents/state/KERNEL.md`，只交给 `plan`。

## Invariants

- 语义变化先经过 `plan`；第一次 `start` 建立缺失的 confirmed-only `K1` 是
  初始化例外，不能用来修改已有长期语义。
- 项目 `.spec-agents/state/KERNEL.md` 保存项目语义；本文件只保存 SPEC-AGENTS 工作流语义；根
...
- `.spec-agents/specs/` 是被跟踪的长期工作契约，只由项目工作写入，安装器不碰；
  `.spec-agents/doctrine/docs/` 归属相反。引用时写完整路径。`.spec-agents/scratch/` 只放等确认的
  一次性报告；建议项目忽略它，但框架不写项目的 `.gitignore`（ADR 0003）。
- `.spec-agents/state/STATUS.md` 只记录活跃 SPEC、阻塞项和下一步；完成即移除，不保存已关闭的
  段落（ADR 0002）。
- 被管项目的根 `AGENTS.md` 只有包含指向 `.spec-agents/doctrine/AGENTS.md` 的精确集成指令，才能建立现代入口；已有项目自有入口保持不变，缺少该指令时安装只报告 integration required，不能宣称 ready。
- Doctrine 与 Instance 可以共享 `.spec-agents/`，但仍是两个替换边界：replacement 只拥有 `.spec-agents/doctrine/` 和精确生成的根适配器，绝不整体删除 `.spec-agents/`；被管项目中的动作不写这些安装产物，要改就改上游，SPEC-AGENTS 自身是例外（ADR 0004）。
- `do` 只写 `Code` 与当前切片的状态；`.spec-agents/state/KERNEL.md`、`CONTEXT.md`、`.spec-agents/state/STATUS.md`、
  `.spec-agents/state/EVIDENCE.md`、ADR、Protocol、Runbook、Lesson 由 `learn` 写入，SPEC 由
...
- 没有 `.jj/` 的项目不自动初始化；启用 colocated JJ 必须经过用户明确选择
  和 setup Runbook。
- 第一次 `start` 在 `.spec-agents/state/KERNEL.md` 缺失且有直接确认事实时建立 `K1`，只写
  confirmed 内容。其余情况 `start` 在用户确认前只能写
  `.spec-agents/scratch/start/REPORT.md`。
- `start` 不是第七个 action；确认后的现代项目必须进入 `plan`；
  `upgrade-needed` 项目进入 `.spec-agents/doctrine/UPGRADE.md`，清场后重新 START，不继承旧工作状态。
- `StartReport` 必须区分 confirmed、inferred 和 unknown；未确认的候选认知
  不能进入 `.spec-agents/state/KERNEL.md` 的 enacted sections 或默认上下文。
- 已有 `.spec-agents/state/KERNEL.md` 时，`start` 只能报告冲突或陈旧，不得静默覆盖；首次 K1

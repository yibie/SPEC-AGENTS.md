...
reverse trade-offs to `docs/adr/`, and current state to `STATUS.md`. If an
issue has `evidence_ref`, append the Evidence ID first, then write the same ID
back to the issue; `learn` is the only writer.

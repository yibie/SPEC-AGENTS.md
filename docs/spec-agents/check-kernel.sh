...
#
# Usage:  docs/spec-agents/check-kernel.sh [project-root]
...
if [ ! -f "$KERNEL" ]; then
  note "no KERNEL.md in $ROOT — nothing to check."
  exit 0
fi

section=$(awk '/^## Architecture boundaries[[:space:]]*$/{f=1;next} /^## /{f=0} f' "$KERNEL")
if [ -z "$(printf '%s' "$section" | tr -d '[:space:]')" ]; then
  note "KERNEL.md has no Architecture boundaries entries — the authority map is a gap, not a failure."
  exit 0
fi
